...
class functions {
  ...
  public function SendTradeRequest($leagueid, $ssid, $oppteamid, $addID, $dropID){
    ...
    public function ReadyDraft($leagueuserid){
        $update = "UPDATE league_user SET status=1 WHERE id='$leagueuserid'";
        $this->_mysqli->query($update);
    }
    ...
    public function GetRosterStatus(){
        ...
        while($row = $result->fetch_assoc()){
            ...
            $query = "SELECT ssid FROM users WHERE user_id='$currentID'";
            $res = $this->_mysqli->query($query);
            $ssid = '';
            while($ro = $res->fetch_assoc()){
                $ssid = $ro['ssid'];
            }
            $return[] = [$row['team_name'], $row['id'], $row['status'], $ssid];
        }
        return json_encode($return);
    }
