<section id="landing">
    ...
    $table = "temp_invite";
    $field = "linkid";
    $value = $pg['invite_info'];
    $get_data = $functions->getDataQuery($table, $field, $value);
    $get_data = $functions->SetDataQuery();

    //var_dump($get_data);
    foreach ($get_data as $information) {
    ...
    $table_user = "users";
    $field_user = "email";
    $value_user = $information['email'];
    unset($functions->_data);
    $check_if_registered = $functions->getDataQuery($table_user, $field_user, $value_user);

    $check_if_registered = $functions->SetDataQuery();
    foreach ($check_if_registered as $checked){}
    $name = ($checked['first_name'] == "")?$information['name'] : $checked['first_name'];
    $is_registered = count($check_if_registered);

    if ($is_registered == 1) {
        $class1 = "mfp-hide";
        $class2 = "";
        $_POST['form']['login']['email'] = $information['email'];
    } else {
        $class2 = "mfp-hide";
        $class1 = "";
        $_POST['form']['signup']['firstname'] = $information['name'];
        $_POST['form']['signup']['email'] = $information['email'];
    }
    ?>
    <div class="banner">
        <div class="container">
            <div class="banner-info wow bounceIn" data-wow-delay="0.4s">
                ...
                <div id="small-dialog2" class="<?= $class1 ?>">
                    <?php
                    echo $forms->SignUpForm();
                    //$forms->SignUpProcess($pg['signup']);
                    ?>
                </div>	
                <div id="small-dialog3" class="<?= $class2; ?>">
                    <?php
